...
def check_for_monster(grid, grid_start, row_start):
	for i in range(0, monsterheight):
		for j in range(0, monsterwidth):
			if monster[i][j] == "#":
				if grid[grid_start + i][row_start + j] != "#":
					return False
	return True

def count_churn():
	count = 0
	tester = picture.get_lines() 
	for row in tester:
		for char in row:
			if char == "#":
				count += 1
...
def count_monsters():
	...
	for ori in orientations():
		for flipped in [True, False]:
			picture.orientation = ori
			picture.flipped = flipped
			tester = picture.get_lines() 
			i = 0
			while i < picture.height - monsterheight:
				j = 0
				while j < picture.width - monsterwidth:
					if check_for_monster(tester, i, j):
						count += 1
						# print("found", ori, flipped, i, j)
						j += monsterwidth
					j += 1
				i += 1
			if count > 0:
...
# 	if check_for_monster():
# 	for j in range(0, len(monster)):

result = count_churn() - (count_monsters() * monstersize)
