...
def find_paths(start, prefix, end, seen_smalls, lock):
	...
	for node in start.connections:
		...
		paths.update(find_paths(node, prefix, end, seen_smalls, lock))
		if node.issmall:
			seen_smalls.remove(node)
			if lock is None:
				paths.update(find_paths(node, prefix, end, seen_smalls, node))
...
paths = find_paths(start, "", end, set([start]), None)
# for path in paths:
# 	print path
result = len(paths)
